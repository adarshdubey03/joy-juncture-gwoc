...
import './wooden-cart-button.css';

interface WoodenCartButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
    text?: string;
}

export function WoodenCartButton({ text = "Add to cart", className, ...props }: WoodenCartButtonProps) {
    return (
        <div className={`inline-flex ${className || ''}`}>
            <button className="wooden-cart-button" {...props}>
